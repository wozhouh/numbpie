...
class SimilaritySearcher:
    ...
    def __init__(self, img_dir, features_path, dst_dir, sim_dir, num_display=10):
    ...
    def find_similar_idx(self, feature_q, top_k=20, clip_th=0.78):
        ...
        top_idx_clip = [top_idx[i] for i in range(top_k) if sim[0, top_idx[i]] >= clip_th]  # select by similarity threshold
    ...
    def copy_img2dir(self, idx):
        ...
        for k, i in enumerate(idx):
            ...
            dst_path = os.path.join(self.dst_path, str(k)+'.jpg')
            open(dst_path, 'wb').write(open(img_path, 'rb').read())

    def save_sim(self, sim_list):
        sim_print_list = sim_list[: self.num_display]
        for k, i in enumerate(sim_print_list):
            txt_path = os.path.join(self.sim_dir, str(k)+'.txt')
            f_w = open(txt_path, 'w')
            f_w.write(str(i))
